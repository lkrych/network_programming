...
* [Server and Client Streaming Examples](#client-server-examples)
## What is a socket?
...
<img src='resources/server-client-system-calls.png'>
...
```
1. sockfd - the file descriptor of the connected stream socket.
2. addr - a pointer to a struct that will hold the information about the other side of the connection.
3. addrlen - a pointer to an int that should be initialized to the size of addr.
...
```
1. hostname - a pointer to an array of chars that will contain the hostname upon the functions return.
2. size - the length in bytes of the hostname array.
...
Here is an example of a server that sends the string 'Hello, world!'.
...
The `sigaction()` code is responsible for reaping zombie processes that will appear as the `fork()`ed child processes exit. 

Let's take a look at a simple stream client. 
